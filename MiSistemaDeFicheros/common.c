...
int escribeMapaDeBits(MiSistemaDeFicheros* miSistemaDeFicheros) {
    if (lseek(miSistemaDeFicheros->discoVirtual, TAM_BLOQUE_BYTES * MAPA_BITS_IDX, SEEK_SET) == (off_t) - 1) {
...
int escribeNodoI(MiSistemaDeFicheros* miSistemaDeFicheros, int numNodoI, EstructuraNodoI* nodoI) {
    ...
    if (lseek(miSistemaDeFicheros->discoVirtual, posNodoI, SEEK_SET) == (off_t) - 1) {
        perror("Falló lseek en escribeNodoI");
        return -1;
    }
    if (write(miSistemaDeFicheros->discoVirtual, nodoI, sizeof (EstructuraNodoI)) == -1) {
...
int escribeSuperBloque(MiSistemaDeFicheros* miSistemaDeFicheros) {
    if (lseek(miSistemaDeFicheros->discoVirtual, TAM_BLOQUE_BYTES * SUPERBLOQUE_IDX, SEEK_SET) == (off_t) - 1) {
        perror("Falló lseek en escribeSuperBloque");
        return -1;
    }
    if (write(miSistemaDeFicheros->discoVirtual, &(miSistemaDeFicheros->superBloque), sizeof (EstructuraSuperBloque)) == -1) {
...
int escribeDirectorio(MiSistemaDeFicheros* miSistemaDeFicheros) {
    if (lseek(miSistemaDeFicheros->discoVirtual, TAM_BLOQUE_BYTES * DIRECTORIO_IDX, SEEK_SET) == (off_t) - 1) {
        perror("Falló lseek en escribeDirectorio");
        return -1;
    }
    if (write(miSistemaDeFicheros->discoVirtual, &(miSistemaDeFicheros->directorio), sizeof (EstructuraDirectorio)) == -1) {
...
int escribeDatos(MiSistemaDeFicheros* miSistemaDeFicheros, int archivoExterno, int numNodoI) {
    ...
    for (i = 0; i < temp->numBloques - 1; i++) {
        ...
        if (lseek(miSistemaDeFicheros->discoVirtual, temp->idxBloques[i] * TAM_BLOQUE_BYTES, SEEK_SET) == (off_t) - 1) {
...
int buscaNodoLibre(MiSistemaDeFicheros* miSistemaDeFicheros) {
    int i;
    for (i = 0; i < MAX_NODOSI; i++) {
        if (miSistemaDeFicheros->nodosI[i]->libre==0)
            return i;
    }
    return -1; // NO hay nodos-i libres. Esto no debería ocurrir.
}
